...
if ($argc != 2 || in_array($argv[1], array('--help', '-help', '-h', '-?')))
{
	...
	else
	{
		...
		if (($Key == 0 ))
		{
			echo  "Auto Update/r/n";
			$sql = "SELECT `KeyNumber` FROM `tb_to_download` WHERE `updated` = 0";
...
//			}
	...
	function image($Key)
	{
		...
		else
		{
			...
			include ( '/var/www/config.php');
			$eqsl = "http://www.eqsl.cc/qslcard/GeteQSL.cfm?UserName=$myCall&Password=$password&CallsignFrom=$Call&QSOBand=$Band&QSOMode=$Mode&QSOYear=$Year&QSOMonth=$Month&QSODay=$Day&QSOHour=$Hour&QSOMinute=$Minute";
			// echo $eqsl;
