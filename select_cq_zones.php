...
if ($SUBMIT == "true") {
    if ($INPUT == "input_band") {
		$TEXT = $BAND;
		$BAND = safe("%" . $BAND . "%");
		$query ="SELECT $dbnameHRD.cq_zone_to_work.zones as `CQ Zone to Work`, COL_MODE as `Modes`  FROM $dbnameHRD.cq_zone_to_work where COL_BAND like __REPLACE__  order by col_band,col_mode,zones";
		...
		foreach ($id_lookup as $row):
			{
				$fileName = $row['File'];
				$data .=  "<td>" . $row['CQ Zone to Work'] . "</td><td>" . $row['Modes'] . "</td>" . grid_style($i) . PHP_EOL;
				$i++;
	...
	elseif ($INPUT == "input_mode") {
		$TEXT = $MODE;
        	$MODE = safe("%" . $MODE . "%");
		$query ="SELECT $dbnameHRD.cq_zone_to_work.zones as `CQ Zone to Work`, COL_BAND as `Band`  FROM $dbnameHRD.cq_zone_to_work where COL_MODE like __REPLACE__  order by col_band,col_mode,zones";
